...
import { ShareExtension } from 'capacitor-share-extension';// run this as part of the app launch
//External library
...
async function main() {
  ...
  */
  ...
  //Code to allow orbit send app to appear in the share menu
...
async function checkIntent() {
  try {
    ...
    if (result && result.payload && result.payload.length) {
      let fileSet = new Set()
      for (const fileInfo of result.payload)
      {
        const fileFetched = await fetch(fileInfo.webPath)
        const fileBlob = await fileFetched.blob()
        const file = new File([fileBlob], fileInfo.title, { lastModified: Date.now() })
        fileSet.add(file)
      }
      await sendFiles(fileSet)
    }
  } catch (err) {
    console.log(err);
  }
